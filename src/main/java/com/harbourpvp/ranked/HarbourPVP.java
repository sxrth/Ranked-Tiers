...
public class HarbourPVP extends org.bukkit.plugin.java.JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private static final String PROFILE_TITLE = "§8HarbourPVP §7| §bProfile";
    private static final String LEADERBOARD_TITLE = "§8HarbourPVP §7| §6Leaderboard";
    private static final String HISTORY_TITLE = "§8HarbourPVP §7| §dMatch History";
    ...
    @Override public void onEnable() {
        ...
        for (String cmd : List.of("play","stats","profile","leaderboard","history","queue","party","ht")) {
    ...
    @Override public boolean onCommand(CommandSender s, Command c, String label, String[] a) {
        ...
        if (c.getName().equalsIgnoreCase("stats") || c.getName().equalsIgnoreCase("profile")) return stats(s,a);
    ...
    @EventHandler public void inventoryClick(InventoryClickEvent e) {
        ...
        if (PROFILE_TITLE.equals(title)) {
            e.setCancelled(true);
            if (!(e.getWhoClicked() instanceof Player p) || e.getCurrentItem()==null) return;
            if(e.getRawSlot()==49){openHistoryGui(p);return;}
            if(e.getRawSlot()==53){p.closeInventory();return;}
            return;
        }
        if (LEADERBOARD_TITLE.equals(title)) {
            e.setCancelled(true);
            if (!(e.getWhoClicked() instanceof Player p) || e.getCurrentItem()==null) return;
            int slot=e.getRawSlot();
            if(slot==49){p.closeInventory();return;}
            if(slot>=0 && slot<9){openLeaderboardGui(p,Kit.values()[slot]);return;}
            return;
        }
        if (HISTORY_TITLE.equals(title)) {
            e.setCancelled(true);
            if (!(e.getWhoClicked() instanceof Player p) || e.getCurrentItem()==null) return;
    ...
    private boolean stats(CommandSender s,String[] a){
        Player p=a.length==0&&s instanceof Player?(Player)s:null;
        if(a.length>0)p=Bukkit.getPlayerExact(a[0]);
        if(p==null){s.sendMessage("§cPlayer not found.");return true;}
        if(s instanceof Player viewer && a.length==0){openProfileGui(viewer,p);return true;}
        PlayerData d=store.get(p.getUniqueId(),p.getName());
        s.sendMessage("§6§lHarbourPVP §8» §f"+d.name());
        for(Kit k:Kit.values())s.sendMessage("§e"+k+" §7» §f"+d.rating(k)+" §8» §e"+displayTier(d,k));
        return true;
    }
    private boolean leaderboard(CommandSender s,String[] a){
        Kit k=a.length>0?Kit.from(a[0]):Kit.Sword;
        if(k==null){s.sendMessage("§cUnknown kit.");return true;}
        if(s instanceof Player p){openLeaderboardGui(p,k);return true;}
        List<PlayerData> list=sortedPlayers(k);
        s.sendMessage("§6§l"+k+" Leaderboard");
        int i=1; for(PlayerData pd:list){if(i>10)break;s.sendMessage("§e#"+i+" §f"+pd.name()+" §7» §a"+pd.rating(k)+" §8("+displayTier(pd,k)+")");i++;}
        return true;
    }
    private boolean history(CommandSender s,String[] a){
        if(!(s instanceof Player p)){s.sendMessage("Players only.");return true;}
        openHistoryGui(p); return true;
    }
    private List<PlayerData> sortedPlayers(Kit k){
        List<PlayerData> list=new ArrayList<>(store.all());
        list.sort((x,y)->Integer.compare(y.rating(k),x.rating(k)));
        return list;
    }
    private int wins(PlayerData d){int n=0;for(String h:d.history()){String[] z=h.split("\\|",5);if(z.length>2&&"WIN".equals(z[2]))n++;}return n;}
    private int losses(PlayerData d){int n=0;for(String h:d.history()){String[] z=h.split("\\|",5);if(z.length>2&&"LOSS".equals(z[2]))n++;}return n;}
    private double winrate(PlayerData d){int w=wins(d),l=losses(d);return w+l==0?0.0:(w*100.0/(w+l));}

    private void openProfileGui(Player viewer, Player target){
        PlayerData d=store.get(target.getUniqueId(),target.getName());
        Inventory inv=Bukkit.createInventory(null,54,PROFILE_TITLE);
        inv.setItem(4,item(Material.PLAYER_HEAD,"§b§l"+d.name(),"§7Wins: §a"+wins(d),"§7Losses: §c"+losses(d),"§7Winrate: §e"+String.format(Locale.US,"%.1f",winrate(d))+"%"));
        int[] slots={10,11,12,13,14,15,16,20,22};
        for(int i=0;i<Kit.values().length;i++){Kit k=Kit.values()[i];inv.setItem(slots[i],item(k.icon(this),"§6§l"+k.name(),"§7Rating: §e"+d.rating(k),"§7Tier: §d§l"+displayTier(d,k),"§7Placements: §f"+d.placements(k)+"/5"));}
        inv.setItem(49,item(Material.BOOK,"§d§lMatch History","§7View recent matches"));
        inv.setItem(53,item(Material.BARRIER,"§cClose"));
        viewer.openInventory(inv);
    }
    private void openLeaderboardGui(Player p, Kit selected){
        Inventory inv=Bukkit.createInventory(null,54,LEADERBOARD_TITLE);
        Kit[] kits=Kit.values();
        for(int i=0;i<kits.length;i++){Kit k=kits[i];String mark=k==selected?"§a▶ ":"§7";inv.setItem(i,item(k.icon(this),mark+"§6§l"+k.name(),"§7Click to view leaderboard"));}
        List<PlayerData> list=sortedPlayers(selected);
        for(int i=0;i<Math.min(10,list.size());i++){PlayerData d=list.get(i);Material m=i==0?Material.GOLD_BLOCK:(i==1?Material.IRON_BLOCK:(i==2?Material.COPPER_BLOCK:Material.PAPER));inv.setItem(18+i,item(m,"§e#"+(i+1)+" §f"+d.name(),"§7Rating: §a"+d.rating(selected),"§7Tier: §d§l"+displayTier(d,selected),"§7Wins: §a"+wins(d)+" §8• §7Losses: §c"+losses(d)));}
        inv.setItem(49,item(Material.BARRIER,"§cClose"));
        p.openInventory(inv);
    }
    private void openHistoryGui(Player p){
        PlayerData d=store.get(p.getUniqueId(),p.getName());
        Inventory inv=Bukkit.createInventory(null,54,HISTORY_TITLE);
        int slot=0;
        for(String h:d.history()){if(slot>=45)break;String[] z=h.split("\\|",5);if(z.length<5)continue;Kit k=Kit.from(z[1]);boolean win="WIN".equals(z[2]);inv.setItem(slot++,item(k==null?Material.PAPER:k.icon(this),(win?"§a✔ ":"§c✘ ")+z[1],"§7Opponent: §f"+z[4],"§7Result: "+(win?"§aWIN":"§cLOSS"),"§7Rating: §e"+z[3]));}
        if(slot==0)inv.setItem(22,item(Material.BOOK,"§7No matches yet","§8Play ranked to build history."));
        inv.setItem(49,item(Material.BARRIER,"§cClose"));
        p.openInventory(inv);
    }
    ...
    private boolean admin(CommandSender s,String[] a){if(!s.hasPermission("harbourpvp.admin")){s.sendMessage("§cNo permission.");return true;}if(a.length==0){s.sendMessage("§e/ht setrating <player> <kit> <rating>");s.sendMessage("§e/ht settier <player> <kit> <tier>");s.sendMessage("§e/ht reset <player> <kit>");s.sendMessage("§e/ht forcematch <player1> <player2> <kit>");s.sendMessage("§e/ht arena create <isim>");s.sendMessage("§e/ht arena setspawn <isim> <1|2>");s.sendMessage("§e/ht arena pos1 <isim>");s.sendMessage("§e/ht arena pos2 <isim>");s.sendMessage("§e/ht arena delete <isim>");s.sendMessage("§e/ht arena list");s.sendMessage("§e/ht kit edit <kit>");s.sendMessage("§e/ht kit save <kit>");s.sendMessage("§e/ht kit clear <kit>");s.sendMessage("§e/ht reload");return true;}try{switch(a[0].toLowerCase()){case"setrating"->{if(a.length<4)break;Player p=Bukkit.getPlayerExact(a[1]);Kit k=Kit.from(a[2]);if(p==null||k==null)break;store.get(p.getUniqueId(),p.getName()).rating(k,Integer.parseInt(a[3]));store.save();updateTag(p.getUniqueId());s.sendMessage("§aRating updated.");return true;}case"settier"->{if(a.length<4)break;Player p=Bukkit.getPlayerExact(a[1]);Kit k=Kit.from(a[2]);if(p==null||k==null)break;Integer r=threshold(a[3]);if(r==null){s.sendMessage("§cUnknown tier.");return true;}store.get(p.getUniqueId(),p.getName()).rating(k,r);store.save();updateTag(p.getUniqueId());s.sendMessage("§aTier updated to §e"+a[3]+"§a.");return true;}case"reset"->{if(a.length<3)break;Player p=Bukkit.getPlayerExact(a[1]);Kit k=Kit.from(a[2]);if(p==null||k==null)break;store.get(p.getUniqueId(),p.getName()).rating(k,getConfig().getInt("starting-rating",1000));store.save();updateTag(p.getUniqueId());s.sendMessage("§aReset.");return true;}case"forcematch"->{if(a.length<4)break;Player p1=Bukkit.getPlayerExact(a[1]),p2=Bukkit.getPlayerExact(a[2]);Kit k=Kit.from(a[3]);if(p1==null||p2==null||k==null){s.sendMessage("§cInvalid player/kit.");return true;}startMatch(k,p1.getUniqueId(),p2.getUniqueId());return true;}case"arena"->{
                    ...
                    if(sub.equals("create")){if(a.length<3){s.sendMessage("§e/ht arena create <isim>");return true;}String n=a[2];if(getConfig().isConfigurationSection("arenas."+n)){s.sendMessage("§cBu arena zaten var.");return true;}getConfig().set("arenas."+n+".created",true);getConfig().set("arenas."+n+".spawn1",null);getConfig().set("arenas."+n+".spawn2",null);getConfig().set("arenas."+n+".pos1",null);getConfig().set("arenas."+n+".pos2",null);saveConfig();reloadConfig();s.sendMessage("§aArena oluşturuldu: "+n);return true;}
                    if(sub.equals("setspawn")){if(a.length<4){s.sendMessage("§e/ht arena setspawn <isim> <1|2>");return true;}String n=a[2];String which=a[3];if(!which.equals("1")&&!which.equals("2")){s.sendMessage("§cSpawn 1 veya 2 olmalı.");return true;}if(!getConfig().isConfigurationSection("arenas."+n)){s.sendMessage("§cArena bulunamadı. Önce /ht arena create <isim> yap.");return true;}if(!(s instanceof Player p)){s.sendMessage("§cPlayers only.");return true;}saveLocation("arenas."+n+".spawn"+which,p.getLocation());s.sendMessage("§aArena "+n+" spawn"+which+" ayarlandı.");return true;}
                    if(sub.equals("pos1")||sub.equals("pos2")){if(a.length<3){s.sendMessage("§e/ht arena "+sub+" <isim>");return true;}String n=a[2];if(!getConfig().isConfigurationSection("arenas."+n)){s.sendMessage("§cArena bulunamadı. Önce /ht arena create <isim> yap.");return true;}if(!(s instanceof Player p)){s.sendMessage("§cPlayers only.");return true;}saveLocation("arenas."+n+"."+sub,p.getLocation());s.sendMessage("§aArena "+n+" "+sub+" ayarlandı.");return true;}
